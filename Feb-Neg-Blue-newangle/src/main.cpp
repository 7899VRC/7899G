...
void driveAngleOld(float targetDistanceInches, float angle, float timeout, float wait_ms = 10, float kp = 6.0, float slew = -1, double maxVel = -1) // old code
{
  // old code
...
void driveAngle(double target, double targetAngle, double timeout, double wait_ms = 10, double kp = 6.0, double slew = -1, double maxVel = -1) // march 12 new
{
  // march 12 new
  timer driveAngleTimer;
  left_motor_front.setPosition(0.0, rev);
  double x = 0.0;
  double errorx = target;
  double accuracy = 2;
  double speed = kp * errorx;
  double heading = Inertial.heading(deg);
  double angleError = targetAngle - heading;
  double angleAccuracy = 1.5;
  double ksp = 1.7;
  double steerSpeed = ksp * angleError;
  double prevSpeed = speed;

  while (fabs(errorx) >= accuracy || fabs(angleError) >= angleAccuracy)
  {
    driveVolts(speed + steerSpeed, speed - steerSpeed, 10);

    prevSpeed = speed;
    x = left_motor_front.position(rev) * PI * D * G;
    ...
    if (slew != -1)
    {
      if (speed - prevSpeed > slew)
      {
        speed = prevSpeed + slew;
      }
      else if (speed - prevSpeed < -slew)
      {
        speed = prevSpeed - slew;
      }
    }
    ...
    angleError = targetAngle - heading;
    steerSpeed = ksp * angleError;
    if (driveAngleTimer >= timeout) {
      break;
    }
  }
...
void autonomous(void)
{
  ...
  wait(500, msec);
  driveAngle(22, 270, 700); // arc // changed -90 to 270
  wait(250, msec);
  driveAngle(-38, 240, 1000, 8); // arc back // change 1st paramiter to 34 if bad // changed -120 to 240
